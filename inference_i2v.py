...
def parse_args():
    ...
    parser.add_argument("--no_overlay", action="store_true",
                        help="Skip WASD/IJKL key HUD overlay (for clean video quality eval)")
    ...
    parser.add_argument("--init_image", type=str, default=None,
                        help="I2V: path to a start image. VAE-encode it as the initial "
                             "latent (num_input_frames=1) so generation is conditioned on it.")
...
def load_lora_checkpoint(generator, config, lora_ckpt_path: str):
    ...
    # Coverage guard: peft strict=False silently drops keys outside ckpt-and-mounted
    # ("mounting LoRA on all 180 layers -> cross_attn lora_B stuck at zero init" is
    # exactly this silent behavior; uncovered modules with dW=0 are harmless, but ZERO
    # coverage means rendering the undistilled base — that must be a hard failure).
    ckpt_mods = {k.split(".lora_")[0].replace("base_model.model.", "")
                 for k in gen_lora if ".lora_" in k}
    covered = ckpt_mods & set(target_modules)
    if not covered:
        raise RuntimeError(
            f"LoRA ckpt covers 0/{len(target_modules)} mounted modules — key namespace "
            f"mismatch, would silently render the un-adapted base. "
            f"ckpt module sample: {sorted(ckpt_mods)[:3]}")
    print(f"[lora] coverage: ckpt {len(ckpt_mods)} modules -> covered {len(covered)}"
          f"/{len(target_modules)} mounted (uncovered stay at zero-init dW=0); "
          f"ckpt-only(unmounted)={len(ckpt_mods - set(target_modules))}")
...
def run_dataset_mode(args, config, pipeline, device, local_rank, world_size):
    ...
    for i, batch in tqdm(enumerate(dataloader), total=len(dataloader),
        ...
        idx = batch["idx"].item()
        # The VAE-decode hook moves the generator to CPU to save memory; with multiple dataset prompts it must be moved back to the GPU per sample.
        ...
        # I2V: encode the start image to an initial latent (num_input_frames=1).
        init_latent = None
        if args.init_image:
            from PIL import Image
            import torchvision.transforms.functional as TF
            img = Image.open(args.init_image).convert("RGB").resize((pixel_w, pixel_h), Image.BICUBIC)
            px = TF.to_tensor(img).to(device=device, dtype=torch.bfloat16)  # [3,H,W] in [0,1]
            px = px * 2.0 - 1.0                                             # -> [-1,1] (Wan VAE)
            px = px.unsqueeze(0).unsqueeze(2)                               # [1,3,1,H,W]
            init_latent = pipeline.vae.encode_to_latent(px).to(dtype=torch.bfloat16)  # [1,1,C,h,w]
            init_latent = init_latent.repeat(args.num_samples, 1, 1, 1, 1)
            if local_rank == 0 and i == 0:
                print(f"[I2V] init_image={args.init_image} -> initial_latent {tuple(init_latent.shape)}")
        ...
        if idx < num_prompts:
            for s in range(args.num_samples):
                ...
                if args.no_overlay:
                    vid_with_keys = vid_np            # clean video, no key HUD overlay (for quality evaluation)
                    ...
                    # MULTI-KEY combo format ("wj-wj-wl-..."): keys pressed TOGETHER per
                    # block. parse_key_sequence is single-key (mutually exclusive) and can't
                    # render simultaneous W+J — build per-frame kb+mouse from the combos so
                    # the HUD highlights both keys at once.
                    num_latent = actions.shape[1]
                    combos = [c for c in action_str.split("-") if c]
                    bs = max(1, round(num_latent / max(len(combos), 1)))
                    KBK = set("wasd"); MSK = set("ijkl")
                    kb_seq, mouse_seq = [], []
                    for combo in combos:
                        kb = next((c for c in combo if c in KBK), "q")
                        mo = next((c for c in combo if c in MSK), "u")
                        kb_seq += [kb] * bs; mouse_seq += [mo] * bs
                    kb_seq = (kb_seq + ["q"] * num_latent)[:num_latent]
                    mouse_seq = (mouse_seq + ["u"] * num_latent)[:num_latent]
...
def main():
    ...
    low_memory = get_cuda_free_memory_gb(device) < 40 or os.environ.get("FORCE_LOW_MEM") == "1"
    ...
    # OOM fix: one-shot VAE decode of 765 frames has a huge peak -> save latents + chunked decode (CPU offload). Generation-side OOM is already handled by CFG-off.
    _vae_obj = pipeline.vae
    if hasattr(_vae_obj, "decode_to_pixel_chunk"):
        _lat_dir = os.path.join(args.output_folder, "latents")
        if local_rank == 0:
            os.makedirs(_lat_dir, exist_ok=True)
        _dec_ctr = [0]
        def _decode_save_chunk(latent, use_cache=False):
            try:
                torch.save(latent.detach().to("cpu"), os.path.join(_lat_dir, f"latent_{local_rank}_{_dec_ctr[0]:03d}.pt"))
            except Exception:
                pass
            _dec_ctr[0] += 1
            # Move the DiT to CPU before decoding to free ~10GB so chunked VAE decode fits in the same process (decode is the last step; the DiT is not needed afterwards).
            try:
                pipeline.generator.to("cpu"); torch.cuda.empty_cache()
            except Exception:
                pass
            return _vae_obj.decode_to_pixel_chunk(latent, use_cache=False, chunk_size=24)
        _vae_obj.decode_to_pixel = _decode_save_chunk
